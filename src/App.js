...
main = () => {
  ...
  const trainRL = () => {
    ...
    //Runner('.interstitial-wrapper').restart();
    ...
    //      // console.log("xs[i,4]",xs.length,xs[0].length,xs[i][4]);
    //       if(xs[i][4] === 1) {
    //         let xss =[];
    //         //xs[xs.length-1][4] = 1;
    //         xss.push(xs[i][0] + ii);
      ...
      // for (let i = 0; i < xs.length; i++) { 
         // console.log("xs[i,4]",xs.length,xs[0].length,xs[i][4]);
          //if(xs[i][4] === 1) {
            let xss =[];
            //xs[xs.length-1][4] = 1;
            xss.push(xs[xs.length-1][0] + ii+16);
            xss.push(xs[xs.length-1][1]);
            xss.push(xs[xs.length-1][2]);
            xss.push(xs[xs.length-1][3]);
            xss.push(1);
            xAug.push(xss)
         ...
         // for (let i = 0; i < xs.length; i++) { 
            // console.log("xs[i,4]",xs.length,xs[0].length,xs[i][4]);
             //if(xs[i][4] === 1) {
               let xss =[];
               //xs[xs.length-1][4] = 1;
               xss.push(xs[xs.length-1][0] + ii+15);
               xss.push(xs[xs.length-1][1]);
               xss.push(xs[xs.length-1][2]);
               xss.push(xs[xs.length-1][3]);
               xss.push(1);
           ...
           // for (let i = 0; i < xs.length; i++) { 
              // console.log("xs[i,4]",xs.length,xs[0].length,xs[i][4]);
               //if(xs[i][4] === 1) {
                 let xss =[];
                 //xs[xs.length-1][4] = 1;
                 xss.push(xs[xs.length-1][0] + ii+14);
                 xss.push(xs[xs.length-1][1]);
                 xss.push(xs[xs.length-1][2]);
                 xss.push(xs[xs.length-1][3]);
                 xss.push(1);
             ...
             // for (let i = 0; i < xs.length; i++) { 
                // console.log("xs[i,4]",xs.length,xs[0].length,xs[i][4]);
                 //if(xs[i][4] === 1) {
                   let xss =[];
                   //xs[xs.length-1][4] = 1;
                   xss.push(xs[xs.length-1][0] + ii+17);
                   xss.push(xs[xs.length-1][1]);
                   xss.push(xs[xs.length-1][2]);
                   xss.push(xs[xs.length-1][3]);
                   xss.push(1);
  ...
  //   };
    ...
    //console.log("xAug",xAug);

for (let i=0; i<xAug.length;i++){
  xs.push(xAug[i])
  }

//console.log("xAugBIS",xs)

xAug = [];


const a = tf.data.array(xs).shuffle(xs.length);
...
let ooo = [];
  const aa = async () => {
    await a.forEach(e => oo.push(e));
    ooo.push(oo)
    //console.log("ooo",ooo)

    let tooo = tf.tensor(ooo).squeeze()

    xsFin = tooo.slice([0],[tooo.shape[0],4]).squeeze();
    ysFin = tooo.slice([0,4],[tooo.shape[0],1]).squeeze();
       
    ///console.log("xsFin",xsFin.print(),"ysFin",ysFin.print());

    // Build and compile model.
    myModel = tf.sequential();
    myModel.add(tf.layers.dense({units: 16, inputShape: [4], activation: 'relu'}));
    myModel.add(tf.layers.dense({units: 32, activation: 'relu'}));
    myModel.add(tf.layers.dense({units: 1, activation: 'sigmoid'}));
    myModel.compile({optimizer: 'adam',loss: tf.losses.logLoss});

    // Train model with fit().
    const train = async () => {
      console.log("%c A.I. retraining started",'color: darkred');

      //let pred = document.getElementById("playerInfo");
      pred.innerHTML = "A.I. retraining pending... Please wait a moment"; 

     
     for (let i = 0; i < 2; ++i) {
          const tr = await myModel.fit(xsFin, ysFin, {batchSize: 10, epochs: 10});
          console.log("Loss after Epoch " + i + " : " + tr.history.loss[0]);
          pred.innerHTML = `A.I. retraining pending... Please wait a moment \n Loss after Epoch ${i} : ${tr.history.loss[0]}`; 
          
      }
      
    console.log("%c Retraining finished. Run Dino, run...",'color: darkred');
  ...
  //console.log("muModel after train", myModel);
  Runner('.interstitial-wrapper').restart();
...
aa();
  ...
  if (obstacle){
    ...
    //const xSinglePred = standarize(([[obstacle.xPos, obstacle.yPos, obstacle.width,	spd]]));
      const xSinglePred = tf.tensor(([[obstacle.xPos, obstacle.yPos, obstacle.width,	spd]]));

//console.log("mymodellll",myModel)

    let spr;
    if(myModel !==undefined){
    const SinglePredictiction = async () => {
    const sp = await myModel.predict(xSinglePred);
    spr = sp.round().get(0,0)
    
    if (spr===1){console.log("%c Model predictiction: JUMP",'background: #222; color: red');
        pred.innerHTML = "Jump!";
      }
     else {console.log("Model predictiction: RUN");
     pred.innerHTML = "Run Dino, run";};
  ...
  render() {
    return (
      <div >
        ...
        <h1 className="headline">Self-trained A.I. TRex runner with TFjs</h1>
          <h2 className="subheadline">press space or up to start reinforcement learning</h2>
